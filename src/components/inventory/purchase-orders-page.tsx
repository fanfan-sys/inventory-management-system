...
import {
  Plus, Pencil, Trash2, Search, ShoppingCart, Eye, CheckCircle, XCircle, MinusCircle
} from 'lucide-react'
...
export default function PurchaseOrdersPage() {
  ...
  const { data: orders = [], isLoading } = useQuery<PurchaseOrder[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('获取进货单失败')
      return res.json()
    },
  })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: '创建成功', description: '进货单已创建' })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">进货单管理</h2>
          <p className="text-muted-foreground text-sm">管理所有进货订单，跟踪采购流程</p>
        </div>
        ...
        <CardContent className="p-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
              ...
              <p>暂无进货单数据</p>
              <p className="text-sm">点击"新建进货单"开始创建</p>
            </div>
          ) : (
      ...
      <Dialog open={createOpen} onOpenChange={(open) => { if (!open) resetForm(); setCreateOpen(open) }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>填写进货信息，添加商品明细</DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit((data) => createMutation.mutate(data))} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val) => form.setValue('supplierId', val, { shouldValidate: true })}
              >
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={createMutation.isPending}>
                {createMutation.isPending ? '创建中...' : '创建进货单'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-medium">{detailOrder.supplier?.name || '-'}</span>
                </div>
                <div>
